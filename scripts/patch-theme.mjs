...
const work = path.resolve(getArg("--work", path.join(os.tmpdir(), `workbuddy-skin-${process.pid}`)));
...
if (!patchedHtml.includes(path.basename(skin))) throw new Error(`校验失败：HTML 未注入皮肤链接 ${path.basename(skin)}`);
for (const script of scripts) {
